...
stocksRouter
  ...
  .get(async (req, res) => {
    ...
    try {
      ...
      if (stocks) return res.status(200).send(stocks);
    } catch (error) {
      console.log(error);
    ...
    // stocks.save((err, cust) => {
    //   console.log("abcd ", cust);
    //   if (err) return console.error(err);
    // });
  })
  .post((req, res) => {
    return res.status(400).send("POST operation not supported on /stocks");
  })
  .put((req, res) => {
    return res.status(400).send("PUT operation not supported on /stocks");
  })
  .delete((req, res) => {
    return res.status(400).send("DELETE operation not supported on /stocks");
  });
